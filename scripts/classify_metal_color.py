"""classify_metal_color.py - 用 CLIP 零樣本分析「金工顏色」並存入 DB

金 / 銀 兩種（金色系 = 黃金/玫瑰金，銀色系 = 白金/925銀/鉑金）
共用 embeddings_cache.npz，不需重新讀圖。

執行：python3 scripts/classify_metal_color.py
"""
import json
import sqlite3
import sys
...
def main():
    ...
    metal_prompts  = all_prompts.get('metal_color', {})
    labels_ordered = list(metal_prompts.keys())   # ['金', '銀']
    prompt_texts   = list(metal_prompts.values())

    if not EMBED_CACHE_FILE.exists():
        print("❌ 找不到 embeddings_cache.npz，請先執行其他分類腳本")
        return
    ...
    for (pid,) in rows:
        ...
        conn.execute("UPDATE photos SET metal_color=? WHERE id=?", (best, pid))
        dist[best] += 1
        updated += 1
        if updated % 500 == 0:
            conn.commit()
